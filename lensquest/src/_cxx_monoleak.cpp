void lmoTE(Alm< xcomplex< double > > & alm1, Alm< xcomplex< double > > & alm2, Alm< xcomplex< double > > & almG,  Alm< xcomplex< double > > & almC, PowSpec& wcl, int nside, arr<double> &weight) {
	...
	for (size_t m=0; m<=lmax_alm1; ++m) {
		for (size_t l=m; l<=lmax_alm1; ++l) {
			almG(l,m)=wcl.tt(l)*alm1(l,m);
		}
	} 
	
	almZ.Set(lmax_alm1, lmax_alm1);
	alm2map_spin(job,almG,almZ,map1Q,map1U,0);
	...
	map2alm_spin_iter(job,map1Q,map1U,almG,almC,2,3);
}
...
void lmoEE(Alm< xcomplex< double > > & alm1, Alm< xcomplex< double > > & alm2, Alm< xcomplex< double > > & almG,  Alm< xcomplex< double > > & almC, PowSpec& wcl, int nside, arr<double> &weight) {
	...
	#pragma omp parallel for
	for (int i=0; i< map1Q.Npix(); i++) {
		map3[i]=map2Q[i]*map1Q[i] ;
		map4[i]=map2U[i]*map1Q[i];
	}
	...
	almZ.Set(lmax_alm2, lmax_alm2);
	alm2map_spin(job,almG,almZ,map2Q,map2U,0);
		
	#pragma omp parallel for
	for (int i=0; i< map1Q.Npix(); i++) {
		map1Q[i]=map3[i]+map2Q[i]*map1Q[i];
		map1U[i]=map4[i]+map2Q[i]*map1U[i] ;
	}
