void dirTE(Alm< xcomplex< double > > & alm1, Alm< xcomplex< double > > & alm2, Alm< xcomplex< double > > & almG,  Alm< xcomplex< double > > & almC, PowSpec& wcl, int nside, arr<double> &weight) {
	...
	for (int i=0; i< map1Q.Npix(); i++) {
		...
		map5[i]=  map2U[i]*map1Q[i] - map2Q[i]*map1U[i] ;
		map6[i]=  map2Q[i]*map1Q[i] + map2U[i]*map1U[i] ;
	}
	...
	map2alm_spin_iter(job,map5,map6,almG,almC,1,3);	
	
	#pragma omp parallel for
	for (size_t m=0; m<=lmax_alm1; ++m) {
		for (size_t l=m; l<=lmax_alm1; ++l) {
			tempcomplex=almG(l,m);
			almG(l,m)=almGt(l,m)+complex_i*almCt(l,m)+almG(l,m)-complex_i*almC(l,m);
			almC(l,m)=almGt(l,m)-complex_i*almCt(l,m)+tempdouble+complex_i*almC(l,m);
		}
	} 
}
...
void dirTB(Alm< xcomplex< double > > & alm1, Alm< xcomplex< double > > & alm2, Alm< xcomplex< double > > & almG,  Alm< xcomplex< double > > & almC, PowSpec& wcl, int nside, arr<double> &weight) {
	...
	#pragma omp parallel for
	for (int i=0; i< map1Q.Npix(); i++) {
		map3[i]= map2Q[i]*map1Q[i] + map2U[i]*map1U[i] + map2U[i]*map1Q[i] - map2Q[i]*map1U[i];
		map4[i]=-map2U[i]*map1Q[i] + map2Q[i]*map1U[i] + map2Q[i]*map1Q[i] + map2U[i]*map1U[i];
	}
	...
	#pragma omp parallel for
	for (int i=0; i< map1Q.Npix(); i++) {
		map3[i]+=-map2U[i]*map1Q[i] + map2Q[i]*map1U[i] + map2Q[i]*map1Q[i] + map2U[i]*map1U[i];
		map4[i]+=map2Q[i]*map1Q[i] + map2U[i]*map1U[i] + map2U[i]*map1Q[i] - map2Q[i]*map1U[i];
	}
...
void dirEE(Alm< xcomplex< double > > & alm1, Alm< xcomplex< double > > & alm2, Alm< xcomplex< double > > & almG,  Alm< xcomplex< double > > & almC, PowSpec& wcl, int nside, arr<double> &weight) {
	...
	#pragma omp parallel for
	for (int i=0; i< map1Q.Npix(); i++) {
		map3[i]=-map2U[i]*map1Q[i] + map2Q[i]*map1U[i];
	...
	almZ.Set(lmax_alm2, lmax_alm2);
	alm2map_spin(job,almG,almZ,map2Q,map2U,3);
