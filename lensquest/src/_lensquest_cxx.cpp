		...
		for (size_t l1=lminCMB;l1<lmaxCMB+1;l1++) {
			for (size_t l3=lminCMB;l3<lmaxCMB+1;l3++) {
				if ((l1+L+l3)%2==0) {
					...
					f[tb][l1][l3]=0.0; 
					f[eb][l1][l3]=0.0;
					f[bb][l1][l3]=wcl.cc(l1)*F[l3][l1]+wcl.cc(l3)*F[l1][l3];
				}
				else {
					f[tt][l1][l3]=0.0;
					f[te][l1][l3]=0.0; 
...
void makeA(PowSpec& wcl, PowSpec& dcl, PowSpec& al, size_t lmin, size_t lmax, size_t lminCMB) {
	...
	if (wcl.Num_specs()==4) {num_spec=6; assert(al.Num_specs()>=6);}
	...
	double att, ate, aee, atb, aeb, abb;
	...
	for (size_t L=lmin;L<lmax+1;L++) {
		...
		if (L>=lmin) {
			#pragma omp parallel for reduction(+:att, ate, aee, atb, aeb, abb) schedule(dynamic, 25)
			for (size_t l1=lminCMB;l1<lmaxCMB+1;l1++) {
				for (size_t l3=lminCMB;l3<lmaxCMB+1;l3++) {
					...
					if (num_spec==5) {
						...
						abb+=f[bb][l1][l3]*f[bb][l1][l3]*invlclbb[l1]*invlclbb[l3]*.5;
					}
				}
			}
		}
...
std::vector< std::vector<double> > makeAN(PowSpec& wcl, PowSpec& dcl, PowSpec& rdcls, PowSpec& al, size_t lmin, size_t lmax, size_t lminCMB1, size_t lminCMB2, size_t lmaxCMB1, size_t lmaxCMB2) {
	...
	for (size_t L=lmin;L<lmax+1;L++) {
		...
		att=0.; ate=0.; aee=0.; atb=0.; aeb=0.; abb=0.;
		if (L>=lmin) {
			#pragma omp parallel for reduction(+:att, ate, aee, atb, aeb, abb, ntttt, nttte, nttee, ntete, nteee, neeee, ntbtb, ntbeb, nebeb, nbbbb) schedule(dynamic, 25)
			for (size_t l1=lminCMB1;l1<lmaxCMB1+1;l1++) {
				for (size_t l3=lminCMB2;l3<lmaxCMB2+1;l3++) {
					...
					nbbbb+=f[bb][l1][l3]*invlclbb[l1]*invlclbb[l3]*(f[bb][l1][l3]*invlclbb[l1]*invlclbb[l3]*rdcls.cc(l1)*rdcls.cc(l3)+sgn(L+l1+l3)*f[bb][l3][l1]*invlclbb[l3]*invlclbb[l1]*rdcls.cc(l1)*rdcls.cc(l3))*.25;
				}
			}
		}
